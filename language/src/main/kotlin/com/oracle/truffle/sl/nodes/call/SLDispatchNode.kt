...
abstract class SLDispatchNode : Node() {

    abstract fun executeDispatch(function: Any, arguments: Array<Any>): Any

    /**
     * When no specialization fits, the receiver is not an object (which is a type error).
     */
    @Fallback
    protected fun unknownFunction(function: Any, arguments: Array<Any>): Any {
     ...
     */
    @Specialization(guards = arrayOf("isForeignFunction(function)"))
    protected fun doForeign(function: TruffleObject, arguments: Array<Any>,
            // The child node to call the foreign function
                            @Cached("createCrossLanguageCallNode()") crossLanguageCallNode: Node,
            // The child node to convert the result of the foreign call to a SL value
                            @Cached("createToSLTypeNode()") toSLTypeNode: SLForeignToSLTypeNode): Any {

        try {
            /* Perform the foreign function call. */
            val res = ForeignAccess.sendExecute(crossLanguageCallNode, function, *arguments)
    ...
    companion object {
         ...
         * is overkill, since we could just retrieve it via `function.getCallTarget()`. However,
         * caching the call target and using a [DirectCallNode] allows Truffle to perform method
         ...
         *
         *
         * `limit = "INLINE_CACHE_SIZE"` Specifies the limit number of inline cache specialization
         * instantiations.
         *
         *
         *
         * `guards = "function.getCallTarget() == cachedTarget"` The inline cache check. Note that
         * cachedTarget is a final field so that the compiler can optimize the check.
         *
         *
         *
         * `assumptions = "callTargetStable"` Support for function redefinition: When a function
         ...
         * @param callNode the [DirectCallNode] specifically created for the [CallTarget] in
         * cachedFunction.
         */
        @Specialization(limit = "INLINE_CACHE_SIZE", guards = arrayOf("function.getCallTarget() == cachedTarget"), assumptions = arrayOf("callTargetStable"))//
        //
        protected fun doDirect(function: SLFunction, arguments: Array<Any>,
                               @Cached("function.getCallTargetStable()") callTargetStable: Assumption,
                               @Cached("function.getCallTarget()") cachedTarget: RootCallTarget,
                               @Cached("create(cachedTarget)") callNode: DirectCallNode): Any {
         ...
         */
        @Specialization(replaces = arrayOf("doDirect"))
        protected fun doIndirect(function: SLFunction, arguments: Array<Any>,
        ...
        protected fun isForeignFunction(function: TruffleObject): Boolean {
            return function !is SLFunction
        }

        protected fun createCrossLanguageCallNode(): Node {
            return Message.EXECUTE.createNode()
        }

        protected fun createToSLTypeNode(): SLForeignToSLTypeNode {
