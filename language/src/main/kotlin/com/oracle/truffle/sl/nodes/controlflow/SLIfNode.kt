...
class SLIfNode(conditionNode: SLExpressionNode,
               /** Statement (or [block][SLBlockNode]) executed when the condition is true.  */
               @field:Child private var thenPartNode: SLStatementNode,
               /** Statement (or [block][SLBlockNode]) executed when the condition is false.  */
               @field:Child private var elsePartNode: SLStatementNode?) : SLStatementNode() {

    /**
     * The condition of the `if`. This in a [SLExpressionNode] because we require a
     * result value. We do not have a node type that can only return a `boolean` value, so
