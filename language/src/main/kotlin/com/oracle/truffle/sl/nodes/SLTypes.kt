...
/**
 * The type system of SL, as explained in [SLLanguage]. Based on the [TypeSystem]
...
abstract class SLTypes {
    companion object {
         ...
         * check that the Truffle DSL would generate. For [SLNull], we do not need an
         * `instanceof` check, because we know that there is only a [ singleton][SLNull.SINGLETON] instance.
         */
        @TypeCheck(SLNull::class)
        fun isSLNull(value: Any): Boolean {
         ...
         * that the Truffle DSL would generate. For [SLNull], we do not need an actual cast,
         * because we know that there is only a [singleton][SLNull.SINGLETON] instance.
         */
        @TypeCast(SLNull::class)
        fun asSLNull(value: Any): SLNull {
        ...
        @ImplicitCast
        @TruffleBoundary
        fun castBigNumber(value: Long): SLBigNumber {
            return SLBigNumber(BigInteger.valueOf(value))
        }
    }
}
