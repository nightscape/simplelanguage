...
/**
 * Node to read a local variable from a function's [frame][VirtualFrame]. The Truffle frame API
 * allows to store primitive values of all Java primitive types, and Object values. This means that
 * all SL types that are objects are handled by the [.readObject] method.
 *
 *
...
abstract class SLReadLocalVariableNode : SLExpressionNode() {
    ...
    @Specialization(guards = arrayOf("isLong(frame)"))
    ...
    @Specialization(guards = arrayOf("isBoolean(frame)"))
    protected fun readBoolean(frame: VirtualFrame): Boolean {
        return FrameUtil.getBooleanSafe(frame, slot)
    }

    @Specialization(replaces = arrayOf("readLong", "readBoolean"))
