 ...
 *  1. evaluated the property name
 *  1. reads the named property
 *
 */
@NodeInfo(shortName = ".")
@NodeChildren(
...
abstract class SLReadPropertyNode : SLExpressionNode() {

    @Specialization(guards = arrayOf("isSLObject(receiver)"))
    protected fun read(receiver: DynamicObject, name: Any,
                       @Cached("create()") readNode: SLReadPropertyCacheNode): Any {
        /**
     ...
     */
    @Specialization(guards = arrayOf("!isSLObject(receiver)"))
    protected fun readForeign(receiver: TruffleObject, name: Any,
            // The child node to access the foreign object
                              @Cached("READ.createNode()") foreignReadNode: Node,
            // The child node to convert the result of the foreign read to a SL value
                              @Cached("create()") toSLTypeNode: SLForeignToSLTypeNode): Any {

        try {
            /* Perform the foreign object access. */
            val result = ForeignAccess.sendRead(foreignReadNode, receiver, name)
     ...
     */
    @Fallback
    protected fun typeError(r: Any, name: Any): Any {
